...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const totalInputTokens = ANTHROPIC_FORMATTED_MESSAGES.reduce(
      (acc: number, message: any) => {
        return (
          acc +
          message.content.reduce((contentAcc: number, content: any) => {
    ...
    const anthropic = new Anthropic({
      ...
      baseURL:
        "https://gateway.ai.cloudflare.com/v1/77a0b1436313aeb84549202bdd962b63/pixelverseaisystems/anthropic"
    })

    try {
      const response = await anthropic.messages.create({
        model:
          chatSettings.model === "claude-3-opus-20240229"
